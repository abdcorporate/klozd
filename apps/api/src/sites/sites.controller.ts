...
export class SitesController {
  ...
  @Get()
  @UseGuards(JwtAuthGuard)
  @ApiOperation({ summary: 'Récupérer la liste des sites avec pagination par curseur' })
  ...
  @ApiResponse({ status: 200, description: 'Liste paginée des sites', type: PaginatedResponse })
  findAll(
    ...
    if (user.role === 'SUPER_ADMIN') {
      return this.sitesService.findAllForAdmin(paginationDto);
    }
    ...
    if (user.role === 'ADMIN') {
      ...
      return this.sitesService.findAll(user.organizationId, paginationDto);
    }
