...
export default function EditSitePage() {
  ...
  useEffect(() => {
    ...
    const fetchSite = async () => {
      try {
        ...
        setBackgroundColor(site.backgroundColor || '#ffffff');
        setTextColor(site.textColor || '#000000');
        setPrimaryButtonColor(site.primaryButtonColor || '#000000');
        setFontFamily(site.fontFamily || '');

        // Parser le contentJson
        if (site.contentJson) {
          try {
            const content = JSON.parse(site.contentJson);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Erreur lors de la mise à jour du site');
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <AppLayout>
      <div className="h-[calc(100vh-80px)] flex flex-col bg-gray-50">
        ...
        <div className="flex justify-between items-center px-6 py-4 border-b border-gray-200 bg-white">
          <div>
            <h1 className="text-xl font-bold text-gray-900">Modifier le site</h1>
          </div>
